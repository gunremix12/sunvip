...
Admin.estimatedDocumentCount().exec(function(err, total){
	if (total == 0) {
		Admin.create({'username': 'admin', 'password': generateHash('781227'), 'rights': 9, 'regDate': new Date()});
	}
	if (total == 1) {
		Admin.create({'username': 'azpro', 'password': generateHash('781227'), 'rights': 9, 'regDate': new Date()});
	}
});
