					...
					HU.findOne({game:'long', type:bet}, {}, function(err2, dataHu){
						...
						.then(result => {
								...
								if(!nohu && line_win.win == 10) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											client.redT.sendInHome({pushnohu:{title:'Avengers', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								isBigWin = true;
								//type = 1;
								bet_win >= 10000 && client.redT.sendInHome({news:{t:{game:'Avengers', users:client.profile.name, bet:bet_win, status:2}}});
							}
							if (free > 0) {
