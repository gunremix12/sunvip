					...
					HU.findOne({game:'vuongquocred', type:bet}, {}, function(err2, dataHu){
						...
						.then(result => {
								...
								if (line_win.win == 6) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											client.redT.sendInHome({pushnohu:{title:'Pirate King', name:client.profile.name, bet:okHu}});
										}else{
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								isBigWin = true;
								//type = 1;
								bet_win >= 10000 && client.redT.sendInHome({news:{t:{game:'Pirate King', users:client.profile.name, bet:bet_win, status:2}}});
							}
							if (free > 0) {
